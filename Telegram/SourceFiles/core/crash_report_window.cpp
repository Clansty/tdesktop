...
PreLaunchWindow::PreLaunchWindow(QString title) {
	...
	setWindowTitle(title.isEmpty() ? u"0wGram"_q : title);
...
, _launch(std::move(launch)) {
	...
	if (_sendingState == SendingNoReport) {
		...
		_label.setText(u"Last time 0wGram Desktop crashed :("_q);
	}
	...
	_getApp.setText(u"GET THE LATEST OFFICIAL VERSION OF 0wGram DESKTOP"_q);
	connect(&_getApp, &QPushButton::clicked, [=] {
		QDesktopServices::openUrl(u"https://t.me/clanstty"_q);
	});
